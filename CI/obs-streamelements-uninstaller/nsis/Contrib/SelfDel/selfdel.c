...
* CHANGE HISTORY
*
* 1.0.0.7 - Jan 17 2014 - Stuart Welch (Afrow UK)
*   - Added shutdown/reboot reason for ExitWindowsEx()
*   - Calls InitiateSystemShutdownEx() to reboot fully on Windows
*     Server.
*
* 1.0.0.6 - Jun 28 2013 - Stuart Welch (Afrow UK)
*   - DuplicateHandle missing SYNCHRONIZE permission (thanks
*     kichik)
*
* 1.0.0.5 - Sep 17 2012 - Stuart Welch (Afrow UK)
*   - Added /SHUTDOWN
*
* 1.0.0.4 - Aug 10 2011 - Stuart Welch (Afrow UK)
*   - Added /REBOOT
*
* 1.0.0.3 - Jul 17 2011 - Stuart Welch (Afrow UK)
...
#define SWITCH_SHUTDOWN TEXT("/SHUTDOWN")
...
typedef struct _SELFDEL
{
  ...
  FARPROC fnInitiateSystemShutdownEx;
...
typedef BOOL (WINAPI* PInitiateSystemShutdownEx)(LPTSTR lpMachineName, LPTSTR lpMessage, DWORD dwTimeout, BOOL bForceAppsClosed, BOOL bRebootAfterShutdown, DWORD dwReason);
...
static void remote_thread(SELFDEL *remote)
{
  ...
  for(i = 0; !remote->fnDeleteFile(remote->szFileName) && i < 40; i++)
  {
  ...
  // Afrow UK: reboot/shutdown add-on
  if (remote->fReboot)
  {
    if (remote->fnInitiateSystemShutdownEx == NULL)
      remote->fnExitWindowsEx((remote->fReboot == 2 ? EWX_POWEROFF : EWX_REBOOT) | EWX_FORCE | EWX_FORCEIFHUNG, SHTDN_REASON_FLAG_PLANNED | SHTDN_REASON_MAJOR_SOFTWARE | SHTDN_REASON_MINOR_INSTALLATION);
    else
      remote->fnInitiateSystemShutdownEx(NULL, NULL, 0, TRUE, remote->fReboot != 2, SHTDN_REASON_FLAG_PLANNED | SHTDN_REASON_MAJOR_SOFTWARE | SHTDN_REASON_MINOR_INSTALLATION);
  }
...
NSISFUNC(Del)
{
  ...
  SELFDEL local;
  DWORD entrypoint;
  TCHAR* pszArg = (TCHAR*)GlobalAlloc(GPTR, sizeof(TCHAR) * string_size);
  HMODULE hModule;
  ...
  while (popstring(pszArg) == 0)
  {
    ...
    else if (lstrcmpi(pszArg, SWITCH_SHUTDOWN) == 0)
    {
      local.fReboot = 2;
    }
    else
  ...
  IsWow64Process = (PIsWow64Process)GetProcAddress(hModule, "IsWow64Process");
  ...
  if (bIsWow64Process)
  {
    PWow64EnableWow64FsRedirection Wow64EnableWow64FsRedirection = (PWow64EnableWow64FsRedirection)GetProcAddress(hModule, "Wow64EnableWow64FsRedirection");
...
#ifdef UNICODE
    local.fnInitiateSystemShutdownEx  = (FARPROC)GetProcAddress(hModule, "InitiateSystemShutdownExW");
#else
    local.fnInitiateSystemShutdownEx  = (FARPROC)GetProcAddress(hModule, "InitiateSystemShutdownExA");
#endif

    // Give remote process a copy of our own process handle
    DuplicateHandle(GetCurrentProcess(), GetCurrentProcess(), pi.hProcess, &local.hParent, SYNCHRONIZE, FALSE, 0);
