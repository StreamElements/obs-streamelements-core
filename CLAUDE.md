...
- **Put `#partial` in the title or body of a PR that does not finish its issue.** Linear closes an issue when any linked PR merges, which is wrong when the work is split across several PRs — CORE-554 was closed by the first of three. Nothing can suppress Linear's automation (it fires on the same merge event), so `linear-issue-key.yml`'s `keep-open` job waits for it and restores the issue to In Progress. It only undoes an *automated* close: an issue a human completed is left alone. Needs the `LINEAR_API_KEY` secret; without it the job warns and does nothing.
