...
| --- | --- | --- |
| description | string | Hotkey combination description string (example: “Ctrl + Alt + Shift + S”) |
| keyCode | number | OBS internal key code |
| keyName | string | OBS internal key name (corresponding to *keyCode*) |
| keySymbol | string | Keyboard key symbol (example: “S”, “1”, “BACKSLASH”, “UNDERSCORE”, “RETURN”, “ENTER”, etc.) |
| virtualKeyCode | number | Platform-specific virtual key code. **This roughly maps to the value of** ***KeyboardEvent.keyCode*** **field of ‘keydown’ event.**<br>The full list of virtual key codes available on Windows, can be found here:<br>[https://docs.microsoft.com/en-us/windows/desktop/inputdev/virtual-key-codes](https://docs.microsoft.com/en-us/windows/desktop/inputdev/virtual-key-codes) |
| left | bool | True if modifier keys are pressed on left side of the keyboard |
| right | bool | True if modifier keys are pressed on the right side of the keyboard |
| altKey | bool | Alt key is pressed |
| ctrlKey | bool | Ctrl key is pressed |
| shiftKey | bool | Shift key is pressed |
| commandKey | bool | Command key is pressed |
| capsLock | bool | CAPS lock is on |
| numLock | bool | Num lock is on |
| mouseLeftButton | bool | Left mouse button is pressed |
| mouseMidButton | Bool | Middle mouse button is pressed |
| mouseRightButton | Bool | Right mouse button is pressed |

**Note:** to set key when setting key combinations, use either *keyName* or *virtualKeyCode* field values. *keyName* takes precedence over *virtualKeyCode*.

## Possible values for keyName field
