...
| --- | --- | --- |
| id | string | The event name. Matched case-insensitively by [`setRazerWyvrnEventName`](../host/razer-wyvrn.md#setrazerwyvrneventnamerazerwyvrneventinfo-resultcallbacksuccess). |
...
| fallback | [`RazerWyvrnEventInfo`](RazerWyvrnEventInfo.md) | *Input only.* When passed to `setRazerWyvrnEventName`, the event to try if this one is not declared on the machine. Nests to arbitrary depth. Never present in results. |
